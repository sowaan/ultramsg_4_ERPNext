    ...
    try:
      ...
      if response.status_code == 200:
          ...
          if "sent" in  response_json and  response_json["sent"] == "true":
            ...
            msg1 = frappe.render_template(self.message, context)
            frappe.get_doc({"doctype":"ultramsg_4_ERPNext log","title":"Whatsapp message and pdf successfully sent ","message":msg1,"to_number":number,"time":current_time }).insert()
      ...
      # when the msg send is success then its details are stored into ultramsg_4_ERPNext log  
        if response.status_code == 200:
            ...
            # Log success
              current_time =now()# for geting current time
              msg1 = frappe.render_template(self.message, context)
              frappe.get_doc({"doctype":"ultramsg_4_ERPNext log","title":"Whatsapp message successfully sent ","message":msg1,"to_number":number,"time":current_time }).insert()
            elif "error" in  response_json:
            # Log error
              frappe.log("WhatsApp API Error: " ,  response_json.get("error"))
            else:
